...
erDiagram
  %%========================
  %% 1) Core Lookup Tables - STANDARDIZED
  %%========================
  TAG {
  ...
  STATUS {
    UUID id PK                        "NOT NULL; UNIQUE"
    ENUM name                         "NOT NULL; UNIQUE; CHECK (name IN ('DRAFT', 'PUBLISHED', 'ARCHIVED', 'PENDING', 'APPROVED', 'REJECTED', 'ACTIVE', 'INACTIVE', 'COMPLETED', 'IN_PROGRESS', 'STOPPED', 'INFO', 'DEBUG', 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL'))"
    ...
    SMALLINT sort_order               "NOT NULL; DEFAULT 0; for UI ordering"
    UUID created_by_user_id FK        "NOT NULL; references USER.id"
    UUID updated_by_user_id FK        "NULLABLE; references USER.id"
    TIMESTAMP created_at              "NOT NULL; DEFAULT now()"
    TIMESTAMP updated_at              "NOT NULL; DEFAULT now()"
  ...
  VISIBILITY {
    ...
    SMALLINT permission_level         "NOT NULL; DEFAULT 0; 0=none, 1=view, 2=edit, 3=admin"
    UUID created_by_user_id FK        "NOT NULL; references USER.id"
    UUID updated_by_user_id FK        "NULLABLE; references USER.id"
    TIMESTAMP created_at              "NOT NULL; DEFAULT now()"
    TIMESTAMP updated_at              "NOT NULL; DEFAULT now()"
  ...
  LOG_LEVEL_STATUS {
    ...
    TEXT combination_description      "NULLABLE; describes this specific log/severity combo"
    BOOLEAN requires_immediate_action "NOT NULL; DEFAULT false"
    UUID created_by_user_id FK        "NOT NULL; references USER.id"
    UUID updated_by_user_id FK        "NULLABLE; references USER.id"
    TIMESTAMP created_at              "NOT NULL; DEFAULT now()"
    TIMESTAMP updated_at              "NOT NULL; DEFAULT now()"
    BOOLEAN is_active                 "NOT NULL; DEFAULT true"
    UNIQUE log_severity_combo         "(log_level_status_id, severity_status_id)"
  }
  ...
  MEASUREMENT_UNIT {
    ...
    ENUM unit_name                    "NOT NULL; CHECK (unit_name IN ('KILOGRAM', 'POUND', 'CENTIMETER', 'INCH', 'SECOND', 'MINUTE', 'METER', 'KILOMETER', 'MILE', 'REPETITION', 'PERCENTAGE'))"
    ENUM unit_symbol                  "NOT NULL; CHECK (unit_symbol IN ('kg', 'lb', 'cm', 'in', 's', 'min', 'm', 'km', 'mi', 'reps', '%'))"
    ...
    SMALLINT decimal_places           "NOT NULL; DEFAULT 2; CHECK (decimal_places >= 0 AND decimal_places <= 6)"
    UUID created_by_user_id FK        "NOT NULL; references USER.id"
    UUID updated_by_user_id FK        "NULLABLE; references USER.id"
    TIMESTAMP created_at              "NOT NULL; DEFAULT now()"
    TIMESTAMP updated_at              "NOT NULL; DEFAULT now()"
    BOOLEAN is_active                 "NOT NULL; DEFAULT true"
    UNIQUE metric_unit_name           "(metric_id, unit_name)"
  }
  ...
  PROGRESSION_METRIC {
    ...
    TEXT calculation_formula          "NULLABLE; mathematical formula for progression calculation"
    ENUM progression_direction        "NOT NULL; CHECK (progression_direction IN ('INCREASE', 'DECREASE', 'MAINTAIN', 'VARIABLE'))"
    DECIMAL target_increment          "NULLABLE; CHECK (target_increment > 0); suggested increment per session"
    DECIMAL min_value                 "NULLABLE; minimum allowed value"
    DECIMAL max_value                 "NULLABLE; maximum allowed value"
    UUID created_by_user_id FK        "NOT NULL; references USER.id"
    UUID updated_by_user_id FK        "NULLABLE; references USER.id"
    TIMESTAMP created_at              "NOT NULL; DEFAULT now()"
    TIMESTAMP updated_at              "NOT NULL; DEFAULT now()"
    BOOLEAN is_active                 "NOT NULL; DEFAULT true"
    CHECK min_max_values              "(min_value IS NULL OR max_value IS NULL OR min_value < max_value)"
  }
  ...
  DIFFICULTY_LEVEL {
    ...
    TIMESTAMP updated_at              "NOT NULL; DEFAULT now()"
    BOOLEAN is_active                 "NOT NULL; DEFAULT true"
    UNIQUE metric_level_value         "(metric_id, level_value)"
  }
  
  %%============================
  %% 4) Relationships - FIXED NAMING
  %%============================
  %%— METRIC → its subtypes
  METRIC ||--o{ MEASUREMENT_UNIT      : "has_measurement_units"
  METRIC ||--o{ PROGRESSION_METRIC    : "tracks_progression"
  METRIC ||--o{ DIFFICULTY_LEVEL      : "defines_difficulty"
  
  %%— LOG_LEVEL_STATUS → STATUS (two distinct roles)
  LOG_LEVEL_STATUS }|--|| STATUS      : "log_level_ref"
  LOG_LEVEL_STATUS }|--|| STATUS      : "severity_ref"
  
  %%— Category hierarchy with proper constraints
  CATEGORY ||--o{ CATEGORY            : "parent_category"
  
  %%— Standardized user audit trails
